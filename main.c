...
int main()
{
  ...
  GPIO_PORTC_DEN_R |= 0xF0;
	...
	//second ...portf(LED).............
	// make it act as output and digital.... after that make the led equals "1" which means lock is opened.....lock is opened means LED is "ON"......
	GPIO_PORTF_LOCK_R =0x4C4F434B;  // open the lock...........
  GPIO_PORTF_CR_R = 0x02;
	SET_BIT(GPIO_PORTF_DEN_R,1); //led 1.......
	SET_BIT(GPIO_PORTF_DIR_R,1);
	
	SET_BIT(GPIO_PORTF_DATA_R,1);  // At first the solenoid valve will be unlocked(room is open)(=1) ......
	UART_init();
